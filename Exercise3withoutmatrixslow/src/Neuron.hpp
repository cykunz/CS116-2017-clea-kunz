...
class Neuron {
	private:	
		 ...
		 * 	whether it is refractory or not. 
		 * 	It always has the neurons it is linked to, 
		 * 	the weight of each connection, and whether it is excitatory or not.	
		 * 	For previous exercises, the input also played an important role for
		 * 	each individual neuron.	*/
		
		double input_;	/**<	Input received from environment.	*/
		vector<unsigned int> linked_neurons_;	/**<	Neurons linked to this one, useful for exercise 2 mostly.	*/
		vector<unsigned int> weights_; /**< Weight of each connection in linked_neurons_ */
	...
	Neuron(	double input=0.0, vector<unsigned int> links=vector<unsigned int>(0), vector<unsigned int> weight=vector<unsigned int>(0),
			vector<double> time = vector<double>(0.0), double potential=0.0, int spikes=0, int present=0, bool refractory=false,
...
/***************************************************/
	...
	//! Gets the weights of each connection
	/*! @return Vector of weights associated to each connection	*/
	vector<unsigned int> getWeights() const;
...
/***************************************************/	
	...
	//! Sets the weight at a certain index
	/*! @param idx: Index at which the weight value will be changed
	 * 	@param weight: New weight value.	*/
	void setWeight(unsigned int const& idx, unsigned int const& weight);
...
/***************************************************/
	...
	void addLink(unsigned int const& neuron, unsigned int const& weight);
